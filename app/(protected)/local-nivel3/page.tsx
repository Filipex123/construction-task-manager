...
function LocalPage() {
  ...
  React.useEffect(() => {
    setTitle('Cadastro de Locais Nivel 3');
    setSubtitle('Gerenciar Locais Nivel 3');
    setDescription('Controle e monitore todas os locais de nivel 3 das suas obras');
      ...
      {filteredObras.length === 0 && !isLoading ? (
        ...
        <div className="space-y-6">
          {filteredObras.map((obra) => (
            <LocalNivelCard key={obra.id} obra={obra} onDelete={handleDelete} onUpdate={onUpdateModal} nivel={3}/>
          ))}
        </div>
      )}
